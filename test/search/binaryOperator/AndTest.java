...
class AndTest {
	...
	void setUp() {
		...
		when(tripOne.calculateArrivalDateToTerminal(lima)).thenReturn(LocalDateTime.of(2023, 6, 1, 12, 0));
		...
		when(tripTwo.calculateArrivalDateToTerminal(lima)).thenReturn(LocalDateTime.of(2023, 8, 1, 12, 0));
		when(tripTwo.hasATerminal(lima)).thenReturn(true);
		// -------------------------------------------------------------
